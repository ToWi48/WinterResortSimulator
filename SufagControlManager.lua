...
SufagControlManager.version = 08022021;
...
function SufagControlManager:load(dataTable)  
    ...
    self.setTimeScale = VehicleManager:newFunction("setTimeScale");
    ...
    if dataTable.SufagControlManager.protectionId ~= nil then
    ...
    self.isFixedTower                                                   = dataTable.SufagControlManager.isFixedTower or false;
    ...
    if not self.isMobile and not self.isFixedTower then
    ...
    self.spreadFactor           = dataTable.SufagAutomatic.spreadFactor or 4;
    self.timeFactor             = dataTable.SufagAutomatic.timeFactor or 0.7;
    self.minSpreadRadius        = dataTable.SufagAutomatic.minSpreadRadius or 7;
    self.volumePerSecond        = (dataTable.SufagAutomatic.volumePerHour or 100)/3600; -- in m³ per hour
...
function SufagControlManager:setTimeScale(state)
    self.timescale = state;
    --return "Timescale: " .. tostring(state);
end;
 
function SufagControlManager:update(adt)
    local bdt = adt;
    local timescale = clamp(g_scenario.environment.activeTimescale, 1, 10);
    ...
    self:nonPlayerUpdate(dt, adt, bdt);
    ...
    if player.isActive and self.attacherMasterVehicle == nil and (dx*dx + dy*dy + dz*dz) < self.range then
        ...
        -- todo
        if self.gunLifter ~= nil and self.protectionId ~= nil then
            ...
            if (self.isMobile or self.isFixedTower) and self.protectionId ~= nil then
                g_GUI:addKeyHint(InputMapper.A3D_setCannonProtectionState,  l10n.get(getActive(self.protectionId)   and "A3D_setCannonProtectionFalse" or "A3D_setCannonProtectionTrue"));
                if SufagDevRights:getKeyDown(InputMapper.A3D_setCannonProtectionState) then 
                    self:setProtection(not getActive(self.protectionId));
                end;
        ...
        if self.getPower then
            ...
            -- GunLifter
            if not self.isMobile and not self.isFixedTower and self.protectionId ~= nil then
...
function SufagControlManager:nonPlayerUpdate(dt, adt, bdt)
    -- non player input
    ...
    if ((self.currentMode ~= "OFF") and (self.currentMode ~= "ON")) or not SufagDevRights.isFree then
        ...
        if self.strobeTimer < self.strobeFrequence then
            self.strobeTimer = self.strobeTimer + bdt * 1000;
    ...
    if self.snowSpawning then
        ...
        if g_isMaster then    
            ...
            if deltaHeight >= 4 * SnowSystem.SNOW_HEIGHTSTEP then
...
function SufagControlManager:saveToTable(tbl)
    ...
    if self.protectionId ~= nil then tbl.A3D.SufagControlManager.protection = getActive(self.protectionId); end;
    ...
    if not self.isMobile and not self.isFixedTower then tbl.A3D.SufagControlManager.gunLifterTimer = self.cannonSpec.timer; end;
    ...
    if SufagDevRights.isFree then
        g_GUI:showPopup(l10n.get("A3D_remote_cannon_info_headline"), l10n.get("A3D_remote_cannon_info_desc"),
            ...
            },
            {
                ...
                -- protection
                function() 
                    if self.protectionId ~= nil then
                        if self.cannonSpec.timer == 0 or self.isFixedTower then
                            if self.protectionId ~= nil then self:setProtection(not getActive(self.protectionId)); end;
                            self:snowCannonRemoteCallback();
                        end;
                    end;
                end,
...
function SufagControlManager:popupButtonText(text)
    ...
    if SufagDevRights.isFree then
        if text == "mode" then
            if self.isMobile or self.isFixedTower then
        ...
        elseif text == "protection" then
            if self.protectionId == nil then
                back = l10n.get("A3D_remote_cannon_info_btn_protectionOff");
            else
...
function SufagControlManager:loadFromTable(tbl)
    ...
    if self.protectionId ~= nil then setActive(self.protectionId, getNoNil(tbl.A3D.SufagControlManager.protection, true)); end;
    ...
    if not self.isMobile and not self.isFixedTower then self.cannonSpec.timer = getNoNil(tbl.A3D.SufagControlManager.gunLifterTimer, self.cannonSpec.timer); self:setMovement("GunLifter"); end;
...
function SufagControlManager:writeResync()
    ...
    if self.protectionId ~= nil then streamWriteBool(getActive(self.protectionId)); end
    ...
    if not self.isMobile and not self.isFixedTower then streamWriteFloat(self.cannonSpec.timer); end;
...
function SufagControlManager:readResync()
    ...
    if self.protectionId ~= nil then setActive(self.protectionId, streamReadBool(getActive(self.protectionId))); end;
    ...
    if not self.isMobile and not self.isFixedTower then self.cannonSpec.timer = streamReadFloat(self.cannonSpec.timer); end;
...
function SufagControlManager:convertFromSyncro(mode)
  ...
  function SufagControlManager:setProtection(state, noEventSend)
    if self.protectionId == nil then return end;
